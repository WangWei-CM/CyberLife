...
func main() {
	...
	server := &http.Server{Addr:cfg.Address, Handler:httpapi.New(cfg,authService,admin.New(store.Global(),store),nowservice.New(store),aclService,interaction.New(store),history.New(store,aclService),future.New(store)).Router(), ReadHeaderTimeout:10*time.Second}
