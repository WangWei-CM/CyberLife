...
func (s *Server) Router() *gin.Engine {
	...
	writer.GET("",s.nowToday);writer.GET("/mood-tags",s.moodTags);writer.POST("/mood-tags",s.addMoodTag);writer.POST("/moods",s.addMood);writer.POST("/body",s.addBody);writer.POST("/diary/attachments",s.uploadAttachment);writer.PUT("/diary/draft",s.saveDraft);writer.PUT("/diary",s.saveDiary);writer.POST("/tasks",s.addTask);writer.POST("/tasks/:id/done",s.setTaskDone)
...
func (s *Server) uploadAttachment(c *gin.Context){c.Request.Body=http.MaxBytesReader(c.Writer,c.Request.Body,20<<20+1024);file,header,e:=c.Request.FormFile("file");if e!=nil{fail(c,400,"file_rejected","请选择小于 20MB 的附件");return};defer file.Close();a:=c.MustGet("actor").(auth.Actor);x,e:=s.now.SaveAttachment(c.Request.Context(),a.LifeID,header.Filename,header.Header.Get("Content-Type"),header.Size,file);if e!=nil{fail(c,400,"file_rejected",e.Error());return};c.JSON(201,x)}
func (s *Server) saveDraft(c *gin.Context){var r struct{Content string `json:"content"`};if !bind(c,&r){return};a:=c.MustGet("actor").(auth.Actor);x,e:=s.now.SaveDraft(c.Request.Context(),a.LifeID,r.Content);if e!=nil{internal(c,e);return};c.JSON(200,x)}
