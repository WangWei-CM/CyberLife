...
export const api = {
  ...
  createReaderKey: (payload: { nickname: string; note: string; expires_at?: string }) => request<{ reader_key: ReaderKey; key: string }>('/api/v1/now/reader-keys', { method: 'POST', body: JSON.stringify(payload) }),
  revokeReaderKey: (id: string) => request<void>(`/api/v1/now/reader-keys/${encode(id)}/revoke`, { method: 'POST' }),
