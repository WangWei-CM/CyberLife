...
<template>
  <main class="login-page" :class="`login-${phase}`">
    <section v-if="phase !== 'greeting'" class="login-form" @submit.prevent="submit">
      ...
      <form><input v-model="credential" autocomplete="current-password" placeholder="粘贴主密钥或阅读密钥" type="password" :disabled="busy" autofocus /><button aria-label="登录" :disabled="busy">进入</button></form>
