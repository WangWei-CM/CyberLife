...
export function createLoginTransition(canvas: HTMLCanvasElement, options: SceneOptions = {}) {
  ...
  const starCount = mobile ? 10_500 : 26_000
  ...
  const dustCount = mobile ? 4_200 : 9_500
  ...
  const render = (time: number) => {
    ...
    if (stage === 'cloud-gather') {
      ...
      surfaceHaloMaterial.uniforms.uOpacity.value = .1 * earthFade
      ...
      surfaceHaloMaterial.uniforms.uOpacity.value = .1
